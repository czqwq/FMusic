package com.Lilith.FMusic.client.core.player.decoder.m4a.aac.ps;
...
public class PS implements PSConstants, PSTables, HuffmanTables {
    ...
    float[][][][] delay_Qmf_ser = new float[NO_ALLPASS_LINKS][5][64][2]; /*
                                                                          * 5 samples delay max (table 8.34), 64 QMF
                                                                          * channels
                                                                          */
    ...
    public int decode(BitStream ld) throws AACException {
        ...
        if (ld.readBool()) {
            ...
            if (this.enable_iid) {
                ...
                if (this.iid_mode == 2 || this.iid_mode == 5) this.use34hybrid_bands = true;
            ...
            if (this.enable_icc) {
                ...
                if (this.icc_mode == 2 || this.icc_mode == 5) this.use34hybrid_bands = true;
            }
    ...
    private void huff_data(BitStream ld, boolean dt, int nr_par, int[][] t_huff, int[][] f_huff, int[] par)
        throws AACException {
        int n;
    ...
    private void ps_data_decode() {
        ...
        if (this.use34hybrid_bands) {
            for (env = 0; env < this.num_env; env++) {
                if (this.iid_mode != 2 && this.iid_mode != 5) map20indexto34(this.iid_index[env], 34);
    ...
    private void ps_decorrelate(float[][][] X_left, float[][][] X_right, float[][][] X_hybrid_left,
        ...
        for (bk = 0; bk < this.nr_par_bands; bk++) {
            for (n = this.border_position[0]; n < this.border_position[this.num_env]; n++) {
                ...
                if (this.P_PeakDecayNrg[bk] < P[n][bk]) this.P_PeakDecayNrg[bk] = P[n][bk];
        ...
        for (gr = 0; gr < this.num_groups; gr++) {
            if (gr < this.num_hybrid_groups) maxsb = this.group_border[gr] + 1;
            else maxsb = this.group_border[gr + 1];

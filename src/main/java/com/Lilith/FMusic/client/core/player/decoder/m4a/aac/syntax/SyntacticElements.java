...
import com.Lilith.FMusic.client.core.player.decoder.m4a.aac.tools.LTPrediction;
import com.Lilith.FMusic.client.core.player.decoder.m4a.aac.tools.MS;
...
public class SyntacticElements implements Constants {
            ...
            switch (config.getChannelConfiguration()) {
                ...
                default:
                    throw new AACException(
                        "unsupported channel configuration for error resilience: " + config.getChannelConfiguration());
            }
        }
        in.byteAlign();
    ...
    private void decodeFIL(BitStream in, Element prev) throws AACException {
        ...
        if (prev != null && prev.isSBRPresent()) {
            sbrPresent = true;
            if (!psPresent && prev.getSBR()
                .isPSUsed()) psPresent = true;
        }
    }
    ...
    public void process(FilterBank filterBank) throws AACException {
        ...
        int chs = config.getChannelConfiguration()
            .getChannelCount();
    ...
    private int processSingle(SCE_LFE scelfe, FilterBank filterBank, int channel, Profile profile, SampleFrequency sf)
        ...
        // prediction
        if (profile.equals(Profile.AAC_MAIN) && info.isICPredictionPresent()) info.getICPrediction()
            .process(ics, iqData, sf);
        ...
        // TNS
        if (ics.isTNSDataPresent()) ics.getTNS()
            .process(ics, iqData, sf, false);
        ...
        // gain control
        if (ics.isGainControlPresent()) ics.getGainControl()
            .process(
                iqData,
                info.getWindowShape(ICSInfo.CURRENT),
                info.getWindowShape(ICSInfo.PREVIOUS),
        ...
        if (sbrPresent && config.isSBREnabled()) {
            ...
            if (sbr.isPSUsed()) {
                chs = 2;
                scelfe.getSBR()
                    .process(data[channel], data[channel + 1], false);
            } else scelfe.getSBR()
                .process(data[channel], false);
        }
        return chs;
    }
    ...
    private void processPair(CPE cpe, FilterBank filterBank, int channel, Profile profile, SampleFrequency sf)
        ...
        if (profile.equals(Profile.AAC_MAIN)) {
            if (info1.isICPredictionPresent()) info1.getICPrediction()
                .process(ics1, iqData1, sf);
            if (info2.isICPredictionPresent()) info2.getICPrediction()
                .process(ics2, iqData2, sf);
        }
        // IS
        ...
        // TNS
        if (ics1.isTNSDataPresent()) ics1.getTNS()
            .process(ics1, iqData1, sf, false);
        if (ics2.isTNSDataPresent()) ics2.getTNS()
            .process(ics2, iqData2, sf, false);
        ...
        // gain control
        if (ics1.isGainControlPresent()) ics1.getGainControl()
            .process(
                iqData1,
                info1.getWindowShape(ICSInfo.CURRENT),
                info1.getWindowShape(ICSInfo.PREVIOUS),
                info1.getWindowSequence());
        if (ics2.isGainControlPresent()) ics2.getGainControl()
            .process(
                iqData2,
                info2.getWindowShape(ICSInfo.CURRENT),
                info2.getWindowShape(ICSInfo.PREVIOUS),
    ...
    public void sendToOutput(SampleBuffer buffer) {
        ...
        final int freq = mult * config.getSampleFrequency()
            .getFrequency();

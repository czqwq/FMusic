...
import com.Lilith.FMusic.client.core.player.decoder.m4a.aac.ps.PS;
import com.Lilith.FMusic.client.core.player.decoder.m4a.aac.syntax.BitStream;

public class SBR
    implements Constants, com.Lilith.FMusic.client.core.player.decoder.m4a.aac.syntax.Constants, HuffmanTables {
    ...
    void sbr_reset() {
        ...
        this.Reset = (this.bs_start_freq != this.bs_start_freq_prev) || (this.bs_stop_freq != this.bs_stop_freq_prev)
    ...
    int calc_sbr_tables(int start_freq, int stop_freq, int samplerate_mode, int freq_scale, boolean alter_scale,
        ...
        if (this.sample_rate.getFrequency() >= 48000) {
            if ((k2 - this.k0) > 32) result += 1;
        } else if (this.sample_rate.getFrequency() <= 32000) {
            if ((k2 - this.k0) > 48) result += 1;
        } else { /* (sbr.sample_rate == 44100) */

            if ((k2 - this.k0) > 45) result += 1;
        }
    ...
    public int decode(BitStream ld, int cnt) throws AACException {
        ...
        if (this.bs_header_flag) sbr_header(ld);
        ...
        if (this.header_count != 0) {
            if (this.Reset || (this.bs_header_flag && this.just_seeked)) {
                int rt = calc_sbr_tables(
                    this.bs_start_freq,
                    this.bs_stop_freq,
                    this.bs_samplerate_mode,
                    this.bs_freq_scale,
                    this.bs_alter_scale,
                    this.bs_xover_band);
    ...
    private int sbr_data(BitStream ld) throws AACException {
        ...
        if (stereo) {
            if ((result = sbr_channel_pair_element(ld)) > 0) return result;
        } else {
            if ((result = sbr_single_channel_element(ld)) > 0) return result;
        }
    ...
    private int sbr_single_channel_element(BitStream ld) throws AACException {
        ...
        if ((result = sbr_grid(ld, 0)) > 0) return result;
        ...
        if (this.bs_add_harmonic_flag[0]) sinusoidal_coding(ld, 0);
    ...
    private int sbr_channel_pair_element(BitStream ld) throws AACException {
        ...
        if (this.bs_coupling) {
            ...
            if (this.bs_add_harmonic_flag[1]) sinusoidal_coding(ld, 1);
        } else {
        ...
        if (this.bs_coupling) NoiseEnvelope.unmap(this);
    ...
    private int sbr_grid(BitStream ld, int ch) throws AACException {
        ...
        if (this.bs_frame_class[ch] == VARVAR) this.L_E[ch] = Math.min(bs_num_env, 5);
        else this.L_E[ch] = Math.min(bs_num_env, 4);

        if (this.L_E[ch] <= 0) return 1;

        if (this.L_E[ch] > 1) this.L_Q[ch] = 2;
        else this.L_Q[ch] = 1;
    ...
    private void sbr_envelope(BitStream ld, int ch) throws AACException {
        ...
        if ((this.L_E[ch] == 1) && (this.bs_frame_class[ch] == FIXFIX)) this.amp_res[ch] = false;
        else this.amp_res[ch] = this.bs_amp_res;
    ...
    private int sbr_save_prev_data(int ch) {
        ...
        if (this.l_A[ch] == this.L_E[ch]) this.prevEnvIsShort[ch] = 0;
        else this.prevEnvIsShort[ch] = -1;
    ...
    private int sbr_process_channel(float[] channel_buf, float[][][] X, int ch, boolean dont_process) {
        ...
        /* subband analysis */
        if (dont_process) qmfa[ch].sbr_qmf_analysis_32(this, channel_buf, this.Xsbr[ch], this.tHFGen, 32);
        else qmfa[ch].sbr_qmf_analysis_32(this, channel_buf, this.Xsbr[ch], this.tHFGen, this.kx);
    ...
    public int process(float[] left_chan, float[] right_chan, boolean just_seeked) {
        ...
        if (this.ret != 0 || (this.header_count == 0)) {
            ...
            if (this.ret != 0 && this.Reset) this.bs_start_freq_prev = -1;
        }
        ...
        if (this.bs_header_flag) this.just_seeked = false;

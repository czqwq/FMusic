...
class Filterbank implements PSTables {
    ...
    void channel_filter8(int frame_len, float[] filter, float[][] buffer, float[][][] X_hybrid) {
        ...
        for (i = 0; i < frame_len; i++) {
            ...
            input_re1[2] = -(filter[0] * (buffer[i][0] + buffer[12 + i][0]))
            ...
            input_im1[1] = (filter[0] * (buffer[12 + i][1] - buffer[i][1]))
            ...
            input_im2[2] = -(filter[0] * (buffer[i][1] + buffer[12 + i][1]))
            ...
            input_re2[1] = (filter[0] * (buffer[12 + i][0] - buffer[i][0]))

...
public class HudSave {
    ...
    public static void saveDefaultHud() {
        try {
            String data = FMusic.gson.toJson(defaultHud);
    ...
    public static void loadHud() throws IOException {
        ...
        if (list != null) {
            for (File item : list) {
                if (item.isFile() && item.getName()
                    .endsWith(".json")) {
                    try {
                        InputStreamReader reader = new InputStreamReader(
                            Files.newInputStream(item.toPath()),
                            StandardCharsets.UTF_8);
                        BufferedReader bf = new BufferedReader(reader);
                        HudPosObj hud = FMusic.gson.fromJson(bf, HudPosObj.class);
                        bf.close();
                        reader.close();
        ...
        InputStreamReader reader = new InputStreamReader(
            Files.newInputStream(defaultFile.toPath()),
            StandardCharsets.UTF_8);
        BufferedReader bf = new BufferedReader(reader);
        defaultHud = FMusic.gson.fromJson(bf, HudPosObj.class);
        bf.close();
        reader.close();
        defaultHudCheck();
    }
    ...
    private static void saveHud(String name) {
        try {
            ...
            String data = FMusic.gson.toJson(hud);

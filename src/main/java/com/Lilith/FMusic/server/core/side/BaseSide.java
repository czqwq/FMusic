...
public abstract class BaseSide {
    ...
    public final void sendPos(String player, int pos) {
        ...
        if (FMusic.isSkip(player, getPlayerServer(player), true)) return;
        try {
    ...
    public final void sendHudKtv() {
        MusicPack.LyricKtvMusicPack pack = new MusicPack.LyricKtvMusicPack(
            PlayMusic.musicNowTime,
            PlayMusic.lyric.getKtvNow());
    ...
    public final void sendHudPos(String name) {
        Object player = getPlayer(name);
        if (player == null) return;
        if (FMusic.isSkip(name, null, false)) return;
        try {
            HudPosObj obj = HudSave.getOrNew(name);
    ...
    public final void sendMusic(String url) {
        ...
        for (Object player : getPlayers()) {
            ...
            if (name == null) continue;
            name = name.toLowerCase(Locale.ROOT);
            if (FMusic.isSkip(name, getPlayerServer(player), false)) continue;
            try {

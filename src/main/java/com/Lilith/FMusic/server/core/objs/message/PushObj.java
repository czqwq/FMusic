...
public class PushObj {
    ...
    public void init() {
        ...
        if (agree == null) agree = "<gold>[FMusic]<yellow>你同意插歌";
        if (bqAgree == null) bqAgree = "<gold>[FMusic]<yellow>" + ARG.player
            ...
            + ARG.countAll
            + "名玩家同意才会插歌。";
        if (arAgree == null) arAgree = "<gold>[FMusic]<yellow>你已申请插歌";
        if (timeOut == null) timeOut = "<gold>[FMusic]<yellow>插歌时间结束";
        ...
        if (idErr == null) idErr = "<gold>[FMusic]<red>错误的歌曲序号";
        if (pushErr == null) pushErr = "<gold>[FMusic]<red>这首歌已经是下一首播放了";
        if (err1 == null) err1 = "<gold>[FMusic]<red>你没有发起插歌";
        if (err2 == null) err2 = "<gold>[FMusic]<red>你发起的插歌已超时";
        if (err3 == null) err3 = "<gold>[FMusic]<red>发起失败，已经有人发起了插歌投票";

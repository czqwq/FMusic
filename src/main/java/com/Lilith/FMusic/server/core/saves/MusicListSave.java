...
import com.Lilith.FMusic.server.core.objs.config.MusicListObj;
...
public class MusicListSave {
    ...
    public static void saveMusicList() {
        try {
            String data = FMusic.gson.toJson(musicObj);
    ...
    public static void loadMusic() throws IOException {
        InputStreamReader reader = new InputStreamReader(
            Files.newInputStream(musicFile.toPath()),
            StandardCharsets.UTF_8);
        BufferedReader bf = new BufferedReader(reader);
        musicObj = FMusic.gson.fromJson(bf, MusicListObj.class);
        bf.close();
        reader.close();
        musicListCheck();
    }
    ...
    public static MusicObj readListItem() {
        ...
        return readListItem(FMusic.random.nextInt(count));
    }

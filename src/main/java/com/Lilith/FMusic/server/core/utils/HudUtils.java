...
public class HudUtils {
    ...
    public static HudBasePosObj setHudPos(String player, HudType pos, String x, String y) {
        ...
        if (!Function.isInteger(x) && !Function.isInteger(y)) return null;
    ...
    public static float setHudAlpha(String player, HudType pos, String alpha) {
        ...
        if (!Function.isInteger(alpha)) return -1;
    ...
    public static int setHudHudMaxWidth(String player, HudType pos, String width) {
        ...
        if (!Function.isInteger(width)) return -1;
    ...
    public static void sendHudNowData() {
        ...
        if (PlayMusic.nowPlayMusic == null) {
            info = FMusic.getMessage().hud.emptyMusic;
        } else {
            info = FMusic.getMessage().hud.music.replace(ARG.name, infoLimit(PlayMusic.nowPlayMusic.getName()))
    ...
    public static void sendHudLyricData() {
        ...
        if (obj == null) {
            lyr = FMusic.getMessage().hud.emptyLyric;
            tran = "";
            ktv = "";
        } else {
            String lyric = obj.getLyric();
            String tLyric = obj.getTlyric();
            lyr = FMusic.getMessage().hud.lyric.replace(ARG.lyric, lyric == null ? "" : lyric);
            tran = FMusic.getMessage().hud.tlyric.replace(ARG.lyric, tLyric == null ? "" : tLyric);
            ktv = FMusic.getMessage().hud.klyric.replace(ARG.lyric, lyric == null ? "" : lyric);
        }

        FMusic.side.sendHudLyric(new MusicPack.LyricMusicPack(lyr, tran, ktv));
    }
    ...
    public static boolean setHudEnable(String player, HudType pos, String arg) {
        ...
        if (pos == null) {
            if (have) {
                res = obj.info.enable = obj.lyric.enable = obj.pic.enable = obj.state.enable = value;
    ...
    public static boolean setPicSize(String player, String size) {
        HudPosObj obj = HudSave.getOrNew(player);
        if (!Function.isInteger(size)) return false;
    ...
    public static int setGap(String name, HudType type, String arg) {
        if (!Function.isInteger(arg)) return -1;
